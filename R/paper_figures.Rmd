...
pval1 <- wilcox.test(df[df$dataset=='ASCT','Inv.Simpson'],df[df$dataset=='HD1','Inv.Simpson'])$p.value
pval2 <- wilcox.test(df[df$dataset=='ASCT','Inv.Simpson'],df[df$dataset=='HD2','Inv.Simpson'])$p.value
pval3 <- wilcox.test(df[df$dataset=='HD1','Inv.Simpson'],df[df$dataset=='HD2','Inv.Simpson'])$p.value

ggplot(df, aes(x=stage,y=`Inv.Simpson`,color=dataset)) + 
  geom_boxplot(outlier.shape=NA,position=position_dodge(width=.5),width=.5) + 
  geom_point(aes(group=stage),pch=21,fill='white',color='gray',position=position_dodge(width=.5),size=2) + 
  geom_line(aes(group=donor),color='gray',size=.25) +
  geom_text(data=data.frame(pval=pval1,
                                Inv.Simpson=420,
                                stage='T1',
                                dataset=factor('HD1',levels=c('ASCT','HD1','HD2'))),
                aes(label=stars.pval(pval1)),size=2.5, color='black') +
  geom_text(data=data.frame(pval=pval2,
                            Inv.Simpson=420,
                            stage='T1',
                            dataset=factor('HD2',levels=c('ASCT','HD1','HD2'))),
            aes(label=stars.pval(pval2)),size=2.5, color='black') +
  theme_classic() +
  #scale_y_log10() +
  facet_grid(.~dataset, scales='free_x', space='free_x') +
  scale_color_manual(values=dataset_colors,guide=FALSE) +
...
<!-- compare these results to what we got in the Frontiers paper -->

<!-- ```{r compare_frontiers} -->
<!-- pbmc_frontiers <- readRDS("/data/cephfs-2/mirrored/groups/cubi/projects/2019/2019-12-09_Na_ARTE/obermayer_et_al_tcell_persistence/data/seurat/pbmc_all.rds") -->
<!-- ``` -->
...
```{r plot_tmod,fig.width=12,fig.height=6.}
tmods <- read.csv(file.path('tables','pseudobulk_tmod.csv'),header=1,row.names=1)
tmods %>%
  dplyr::filter(adj.P.Val < 1.e-5, AUC > .7, contrast %in% c('AvsH'),!grepl('MRNA',toupper(Title))) %>% 
